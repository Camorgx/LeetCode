            ...
            for (; ans >= 1; --ans) {
                for (int i = 0; i <= m - ans; ++i)
                    for (int j = 0; j <= n - ans; ++j) {
                        int line_sum = sum[i][j + ans - 1] - (i == 0 ? 0 : sum[i - 1][j + ans - 1])
                            - (j == 0 ? 0 : sum[i][j - 1]) + (i && j ? sum[i - 1][j - 1] : 0);
                        int left_cross = 0, right_cross = 0;
                        for (int k = 0; k < ans; ++k) {
                            int x = i + k, y = j + k;
                            int line = sum[x][j + ans - 1] - (x == 0 ? 0 : sum[x - 1][j + ans - 1])
                                - (j == 0 ? 0 : sum[x][j - 1]) + (x && j ? sum[x - 1][j - 1] : 0);
                            int row = sum[i + ans - 1][y]  - (y == 0 ? 0 : sum[i + ans - 1][y - 1])
                                - (i == 0 ? 0 : sum[i - 1][y]) + (i && y ? sum[i - 1][y - 1] : 0);
                            left_cross += grid[x][y];
